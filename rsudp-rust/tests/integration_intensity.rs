use rsudp_rust::intensity::{IntensityManager, IntensityConfig};
use std::fs::File;
...
fn test_intensity_with_verified_samples() {
    let csv_path = "/home/tisayama/Development/rustrsudp_speckit/rsudp-rust/target/tmp/decoded_samples.csv";
    ...
    if !results.is_empty() {
        for res in results {
            println!("[{}] 計測震度: {:.2} ({})", res.timestamp, res.intensity, res.shindo_class);
        }
    }
}
...
fn test_integration_intensity_fdsnws_mseed() {
    ...
    assert!(stdout.contains("計測震度") || stderr.contains("計測震度"), "Intensity calculation was not triggered");
}