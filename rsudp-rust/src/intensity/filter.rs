...
impl JmaFilter {
    ...
    fn calculate_gain(&self, f: f64) -> f64 {
        ...
        let x10 = x6 * x4;
        let x12 = x6 * x6;
        let high_cut = 1.0 / (1.0 + 0.694*x2 + 0.241*x4 + 0.0557*x6 + 0.009664*x8 + 0.00134*x10 + 0.000155*x12).sqrt();
    ...
    pub fn calculate_intensity(&self, x: &[f64], y: &[f64], z: &[f64]) -> f64 {
        ...
        // Vector composition
        let mut composite: Vec<f64> = fx.iter().zip(fy.iter()).zip(fz.iter())
        ...
        let a_03 = if idx < composite.len() { composite[idx] } else { composite.last().cloned().unwrap_or(0.0) };
