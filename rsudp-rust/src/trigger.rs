...
impl std::fmt::Display for AlertEvent {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f, "[{}] Channel {}: {}", self.timestamp, self.channel, self.message)
    }
}
...
impl TriggerManager {
    ...
    pub fn add_sample(&mut self, id: &str, sample: f64, timestamp: DateTime<Utc>) -> Option<AlertEvent> {
        ...
        state.sta = (1.0 - sta_alpha) * state.sta + sta_alpha * sample.abs();
        state.lta = (1.0 - lta_alpha) * state.lta + lta_alpha * sample.abs();

        let ratio = if state.lta > 0.0 { state.sta / state.lta } else { 0.0 };

        if !state.triggered && ratio > self.config.threshold {
            state.triggered = true;
            state.max_ratio = ratio;
            let message = format!("Trigger threshold {} exceeded (ratio: {:.4}). ALARM!", self.config.threshold, ratio);
            ...
            if ratio < self.config.reset_threshold {
                ...
                let message = format!("Ratio {:.4} fell below reset threshold {}. RESET. Max ratio: {:.4}", 
